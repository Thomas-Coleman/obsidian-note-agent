  ...
  describe '#write' do
    ...
    context 'without folder' do
      it 'writes file to vault root' do
        ...
        expect(File.read(file_path)).to include(content)
      end

      it 'adds auto-generation footer to content' do
        result = writer.write(content: content, title: title)

        file_path = File.join(vault_path, "#{title}.md")
        written_content = File.read(file_path)
        expect(written_content).to include('This note was automatically generated by')
        expect(written_content).to include('Obsidian Note Agent')
      end
    ...
    context 'edge cases' do
      ...
      it 'handles empty content' do
        ...
        file_path = File.join(vault_path, "#{title}.md")
        expect(File.exist?(file_path)).to be true
        # Even with empty content, footer should be added
        expect(File.read(file_path)).to include('Obsidian Note Agent')
      end
      ...
      it 'overwrites content when creating duplicate files' do
        ...
        expect(File.read(file_path)).to include(second_content)

        # First file should still have first content
        first_file_path = File.join(vault_path, "#{title}.md")
        expect(File.read(first_file_path)).to include(first_content)
      end
    end
